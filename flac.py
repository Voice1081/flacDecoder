...
class AudioFile:
    ...
    def parse_picture(self, i):
        ...
        return {'picture type': pic_type, 'mime type': mime_type, 'extension': ext, 'description': descr,
                'width': width, 'height': height, 'color depth': color_depth, 'number of colors': number_of_colors,
                'pic': pic}

    @staticmethod
    def __get_pic_type(block):
        return constants.picture_descr[int.from_bytes(block[0:4], byteorder='big')]
    ...
    def __get_colors(block, ext_len, descr_len):
        ...
        number_of_colors = int.from_bytes(block[24 + ext_len + descr_len:28 + ext_len + descr_len], byteorder='big')
            ...
            while pos < len(file):
                if not (b'\xff\xfb' >= file[pos:pos+2] >= b'\xff\xf8'):
                    ...
                    try:
                        ...
                        block_size, block_size_len = self.__get_block_size(file, pos, length, block_size)
    ...
    def __get_block_size(self, file, pos, length, block_size):
        ...
        if block_size == 6:
            ...
            block_size_len = 1
        if block_size == 7:
            block_size = int.from_bytes(file[pos + 4 + length:pos + 6 + length], byteorder='big') + 1
            block_size_len = 2
    ...
    def make_text(self):
        ...
        if self.picture:
            ...
            for i in range(0, len(self.picture)):
                pictures += constants.pic_text.format(str(i+1), self.picture[i]['picture type'],
